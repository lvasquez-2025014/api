...
#include <Windows.h>
#include <winuser.h>  // ← agrega este para GetAsyncKeyState y VK_*
#include <map>
...
struct tab_state
{
    ImVec4 circle, background, rect, shadow, line, icon, text;
};

bool ImGui::Tab(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg, const ImVec2& size_arg_icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? color_edit4 : c::logo_in_active, g.IO.DeltaTime * 10.f);

    it_anim->second.circle = ImLerp(it_anim->second.circle, selectable ? color_edit4 : ImColor(150, 150, 150, 177), g.IO.DeltaTime * 10.f);

    it_anim->second.rect = ImLerp(it_anim->second.rect, selectable ? ImColor(33, 33, 33, 255) : ImColor(33, 33, 33, 0), g.IO.DeltaTime * 10.f);
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, selectable ? ImColor(255, 101, 87, 45) : ImColor(150, 150, 150, 47), g.IO.DeltaTime * 10.f);


    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.rect), 7.f);

    //GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 88, bb.Min.y + 16), 6, GetColorU32(c::circle_rect));

    GetWindowDrawList()->AddShadowCircle(ImVec2(bb.Min.x + 13, bb.Min.y + 13), 7.f, GetColorU32(it_anim->second.circle), 36.f, ImVec2(0, 0));

    // GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 13, bb.Min.y + 13), 8.f, GetColorU32(it_anim->second.shadow));

    // GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(6, 6), bb.Min + ImVec2(21, 21), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(6, 6), bb.Min + ImVec2(size_arg_icon.x + 6, size_arg_icon.y + 6), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
...
bool ImGui::Icon_Button(const char* label, ImTextureID image, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    static std::map<ImGuiID, Icon_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, Icon_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.icon = ImLerp(it_anim->second.icon, IsItemActive() || hovered ? c::text_active : c::logo_in_active, g.IO.DeltaTime * 10.f);


    GetWindowDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::main_color4 : c::bg_checkbox_active, g.IO.DeltaTime * 14.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::circle_active : c::circle_in_active, g.IO.DeltaTime * 14.f);
...
bool ImGui::Keybind(const char* label, const char* description, int* key)
{
    ...
    // --- COORDENADAS (TUS POSICIONES ORIGINALES) ---
    const float w = GetContentRegionMax().x - style.WindowPadding.x;

    // --- LÓGICA DE TEXTO (MOVIDA AQUÍ PARA QUE SE MUESTRE AL DIBUJAR) ---
    ...
    const float box_width = ImMax(40.0f, value_size.x + 12.0f);

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect frame_bb(ImVec2(pos.x + w - box_width - 10, pos.y), ImVec2(pos.x + w - 10, pos.y + 20));
    const ImRect total_bb(pos, ImVec2(pos.x + w, pos.y + 20));
    ...
    // 1. Label de la izquierda
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 20, frame_bb.Min.y + 2), GetColorU32(ImGuiCol_Text), label);

    // 2. Cuadro del Keybind (Fondo)
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(20, 20, 20, 255), 4.0f);

    // 3. Borde con coloedit4 (SIEMPRE VISIBLE)
    GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, ImColor(30, 30, 30, 255), 2.0f);

    // 4. Texto de la tecla (Ahora sí mostrará el nombre o los "...")
    PushStyleColor(ImGuiCol_Text, GetColorU32(c::text_in_active_1));
    RenderTextClipped(frame_bb.Min, frame_bb.Max, buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();
